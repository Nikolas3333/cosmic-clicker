...
function createHangarRoomEnvironment(){
    ...
    const createDockTable = (x, z, accentMat, label) => {
        ...
        plaque.position.set(x < 0 ? 2.35 : -2.35, 2.10, 0.145);
...
function rebuildHangarSceneObjects(){
    ...
    if(currentShip && showcaseGroup){
        ...
        const liveItemRaw = findOwnedHangarShipById(player?.selectedShipId || '') || currentShip || getSelectedShipItem?.() || player?.ships?.[0] || null;
