...
function selectHangarShipById(shipId){
    const safeId = String(shipId || '').trim();
    if(!safeId) return false;
    const ships = getOwnedHangarShips();
    const ownedIndex = ships.findIndex(item => String(item?.id || '').trim() === safeId);
    if(ownedIndex < 0) return false;
    hangarState.shipIndex = ownedIndex;
    return selectCurrentHangarShip();
}
...
function bindHangarMovementControls(){
    ...
    document.addEventListener('keydown', (e) => {
        ...
        if(e.code === 'KeyE' && !e.repeat){
            ...
            if(!hoverShip){
                const astroPos = hangarState?.astronautPivot?.position || null;
                if(astroPos){
                    let bestDist = Infinity;
                    (hangarState.supportPlatforms || []).forEach((pad, idx) => {
                        const candidate = getHangarDockShipByIndex(idx);
                        if(!candidate || !pad) return;
                        const wp = new THREE.Vector3();
                        try{ pad.getWorldPosition(wp); }catch(_){ return; }
                        const dist = wp.distanceTo(astroPos);
                        if(dist < bestDist){ bestDist = dist; hoverShip = candidate; }
                    });
                }
            }
            if(hoverShip){
                e.preventDefault();
                selectHangarShipById?.(hoverShip.id);
            }
        }
    });
    document.addEventListener('keyup', (e) => {
...
function createHangarRoomEnvironment(){
    ...
    centerPlaque.position.set(0, 1.82, -4.82);
...
function updateHangarPlatformPrompt(){
    ...
    if(hint){
        hint.textContent = hoverShip
            ? `${hoverShip?.name || 'Корпус'} • E выбрать` 
            : 'Наведи на боковой корабль и нажми E, чтобы выбрать его.';
    }
...
function normalizeHangarShipMesh(shipMesh){
    try{
        ...
        const finalScale = Math.min(scaleX, scaleY, scaleZ, 2.7);
...
function rebuildHangarSceneObjects(){
    ...
    if(currentShip && showcaseGroup){
        ...
        instantMesh.scale.setScalar(1.08);
        instantMesh.userData.appearStartedAt = performance.now();
        instantMesh.userData.appearTargetScale = 1.28;
        ...
        buildHangarShipMeshAsync(liveItem || currentShip)
            .then((shipMesh) => {
                ...
                shipMesh.scale.setScalar(1.06);
                shipMesh.userData.appearStartedAt = performance.now();
                shipMesh.userData.appearTargetScale = 1.24;
    ...
    dockShips.forEach((ship, dockIndex) => {
        ...
        buildHangarShipMeshAsync(ship)
            .then((sideMesh) => {
                ...
                sideMesh.scale.setScalar(0.98);
                sideMesh.userData.hangarDockIndex = dockIndex;
                sideMesh.userData.baseScale = 0.98;
...
function ensureHangarRenderer(){
    ...
    const animate = () => {
        ...
        if(floatingSellBtn && hangarState.camera){
            ...
            const hoverShip = Number.isFinite(hoverIndex) && hoverIndex >= 0 ? getHangarDockShipByIndex(hoverIndex) : null;
            if(hoverShip){
                const worldPos = getHangarDockWorldPosition(hoverIndex).clone();
                worldPos.y += 0.35;
                ...
                floatingSellBtn.style.top = `${screenY + 56}px`;
...
function bindHangarStageInteraction(){
    ...
    const updateDockHover = (clientX, clientY, activateClick = false) => {
        ...
        if(activateClick && hoverIndex >= 0){
            ...
            if(nextShip){
                selectHangarShipById?.(nextShip.id);
            }
        }
    };
            ...
            <section class="hangar-stage-wrap full no-ship-arrows">
              <div class="hangar-stage">
                ...
                <div class="hangar-stage-overlay" style="pointer-events:none;">
                  <div id="hangar-platform-hint" style="position:absolute;left:50%;bottom:24px;transform:translateX(-50%);padding:10px 16px;border-radius:14px;background:rgba(4,10,22,0.76);border:1px solid rgba(98,216,255,0.25);box-shadow:0 0 18px rgba(0,180,255,0.16);font-size:14px;color:#dff7ff;pointer-events:none;white-space:nowrap;">Наведи на боковой корабль и нажми E, чтобы выбрать его.</div>
